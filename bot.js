...
const {Message, StringSession, Image, Video} = require('./Jinu/');
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('💖 Jinu Mwol working ' + config.WORKTYPE + ' 𝚗𝚘𝚠 '));
          // thanks to Jinu
...
*/
                        try {
                            ...
                            if (config.NOLOG == 'off') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ 𝙹𝙸𝙽𝚄 .𝙼𝚆𝙾𝙻 ~______~*' +
